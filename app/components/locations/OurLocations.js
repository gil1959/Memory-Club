    ...
    <section id="locations" className={clsx('w-full', 'py-8', 'md:py-10', 'relative', 'z-10', 'bg-cream')} style={{ backgroundImage: "url('/bg2locations.png')", backgroundSize: "cover", backgroundPosition: "center" }}>
      <div className={clsx('max-w-[1200px]', 'mx-auto', 'px-4', 'lg:px-8', 'flex', 'flex-col', 'items-center')}>
        ...
        <div className="flex flex-col items-center mb-10 w-full">
          <div className="flex items-center w-full justify-center gap-6 mb-2">
            <div className="flex-1 h-px bg-[#1A1A1A]/30"></div>
            <h2 className={clsx('font-anton', 'text-4xl', 'md:text-5xl', 'uppercase', 'text-[#1A1A1A]')}>
              OUR LOCATIONS
            </h2>
            <div className="flex-1 h-px bg-[#1A1A1A]/30"></div>
          </div>
          <p className={clsx('font-inter', 'text-sm', 'md:text-base', 'text-[#1A1A1A]/80')}>
          ...
          {cities.map((city) => (
            <div key={city.id} className="w-full flex flex-col border border-[#1A1A1A]/20 rounded-lg overflow-hidden bg-white/5 backdrop-blur-sm transition-all duration-300">
              ...
              <button 
                ...
                className={clsx(
                  'w-full flex items-center justify-between py-5 px-6 transition-colors',
                  city.status === 'active' ? 'hover:bg-black/5 cursor-pointer' : 'opacity-70 cursor-default'
                )}
              >
                <div className="flex items-center gap-4">
                  <svg className="w-6 h-6 md:w-7 md:h-7 text-[#9C382A]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                  <span className={clsx('font-anton', 'text-2xl', 'md:text-3xl', 'text-[#1A1A1A]', 'mt-1')}>{city.name}</span>
                </div>
                   ...
                   <div className={clsx(
                     ...
                     {city.venues.map((venue) => (
                       <div key={venue.id} className={clsx(
                         "flex flex-col p-4 border border-[#1A1A1A]/20 bg-white/30 rounded shadow-sm hover:shadow-md transition-shadow",
                         city.venues.length === 3 ? "w-full sm:flex-1 sm:min-w-[280px] sm:max-w-[320px]" : ""
                       )}>
                         <div className="w-full aspect-[16/9] mb-4 relative rounded overflow-hidden border border-[#1A1A1A]/20">
                            <Image src={venue.image} alt={venue.name} fill className="object-cover" />
                         </div>
                         <h4 className="font-anton text-[22px] leading-tight text-[#1A1A1A] mb-1">{venue.name}</h4>
                         <p className="font-inter text-xs text-[#1A1A1A]/80 mb-5 leading-relaxed flex-1">{venue.address}</p>
                         <a href="#" className="font-inter text-[10px] font-bold text-[#1A1A1A] uppercase tracking-[0.2em] border-b border-[#1A1A1A] pb-0.5 w-fit hover:text-[#9C382A] hover:border-[#9C382A] transition-colors">
                           VIEW DETAILS &rarr;
                         </a>
                       </div>
                     ))}
